...
const visaTypes = [
  {
    label: "14-Day Tourist Visa (Adult, Single Entry)",
    price: 25000,
    currency: "LKR"
  },
  {
    label: "14-Day Tourist Visa (Child Under 12, Single Entry)",
    price: 20000,
    currency: "LKR"
  },
  {
    label: "30-Day Tourist Visa (Adult, Single Entry)",
    price: 35000,
    currency: "LKR"
  },
  {
    label: "30-Day Tourist Visa (Child Under 12, Single Entry)",
    price: 20000,
    currency: "LKR"
  },
  {
    label: "30-Day Tourist Visa (Child Under 12, Multiple Entry)",
    price: 35000,
    currency: "LKR"
  },
  {
    label: "60-Day Tourist Visa (Adult, Single Entry)",
    price: 50000,
    currency: "LKR"
  },
  {
    label: "60-Day Tourist Visa (Adult, Multiple Entry)",
    price: 50000,
    currency: "LKR"
  },
  {
    label: "60-Day Tourist Visa (Child Under 12, Multiple Entry)",
    price: 50000,
    currency: "LKR"
  }
];
...
function Application() {
  ...
  useEffect(() => {
    if (appId) {
      console.log("Updated App ID from state:", appId);
    }
  }, [appId]);
  ...
  const handleChange = (e) => {
    ...
    if (name === 'visaType') {
      const selectedVisa = visaTypes.find(type => type.label === value);
      setPrice(selectedVisa.price)
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5001/api/visa/upload", data, { headers: { "Content-Type": "multipart/form-data" } })
                ...
                <Card sx={{ p: 4 }}>
                      ...
                      >
                        ...
                        {visaTypes.map((type, index) => (
                          <MenuItem key={index} value={type.label}>
                            {type.label} - {type.price.toLocaleString("en-IN")}{type.currency}
                          </MenuItem>
                        ))}
                      </Select>
                  ...
                  {step === 5 && (
                    <Elements stripe={stripePromise}>
                      <PaymentForm applicationId={appId} payment={price} />
                    </Elements>
                  )}

                  <Box mt={3} display="flex" justifyContent={step > 1 ? "space-between" : "flex-end"}>
                    {step > 1 && step!==5 && <Button startIcon={<ArrowBackIcon />} onClick={handlePrevious}>Back</Button>}
